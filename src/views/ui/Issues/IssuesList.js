...
export default function IssuesList({ officers }) {
	...
	const getuserByUnit = async unit => {
		try {
			let result = await fetch(
				`http://localhost:5000/api/v1/users/get-users-by-unit/${unit}`,
				{
					headers: {
			...
			response.data
				? setOfficersInUnit(response.data)
				: setOfficersInUnit([])
		} catch (error) {
			console.log(error)
		}
	...
	const superUserColumns = [
		...
		{ field: "issue_name", headerName: "Issue Type", width: 120 },
		{
			field: "assign_by",
			headerName: "Assign By",
			valueGetter: params =>
				params.row.assign_by == userObject.staffId ? `You` : ""
		},
		{
			field: "surname",
	...
	const userColumns = [
		{ field: "id", headerName: "#", width: 10 },
		{ field: "issue_description", headerName: "Description", width: 350 },
	...
	return (
		<>
			...
			<Modal show={showResolve} onHide={handleResolveClose}>
